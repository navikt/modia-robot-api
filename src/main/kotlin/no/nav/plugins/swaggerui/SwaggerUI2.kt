...
@Suppress("unused")
class SwaggerUI2(val config: Configuration) {
    ...
    companion object Feature : ApplicationFeature<Application, Configuration, SwaggerUI2> {
        ...
        override val key: AttributeKey<SwaggerUI2> = AttributeKey("SwaggerUI")

        override fun install(pipeline: Application, configure: Configuration.() -> Unit): SwaggerUI2 {
            pipeline.routing { }.let { routing ->
                ...
                return SwaggerUI2(config = configuration)
            }
        }
    }
}
